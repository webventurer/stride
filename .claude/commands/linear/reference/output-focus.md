...
person, not a changelog: one simple sentence saying what the feature does,
concrete bullets saying exactly what changed, then a short paragraph explaining
how it works. This three-part shape is mandatory and overrides brevity or
compression modes, including Ponytail and Caveman. Follow the
[`/clear-speak` skill](../../../skills/clear-speak/SKILL.md) and write so a
16-year-old who does not know the codebase can understand it on the first read.
...
How it works:
<one short plain-English paragraph explaining the finished behaviour>
...
Diffity: <url>
Linear: In Review
[Squashed N commits into M — if step 12 grouped commits]

Does this look right?
```

When the change is internal, say that plainly in the opening sentence instead
of inventing a user-visible benefit. Keep the explanation of how it works at
the same plain-English level rather than turning it into an implementation log.
