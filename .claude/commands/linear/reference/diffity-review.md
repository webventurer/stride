...
The shared launch procedure for surfacing a PR diff in diffity — a localhost diff viewer, independent of the editor's PR panel, so the visual diff always renders. Called by [`/linear:start`](../start.md) and [`/linear:quick`](../quick.md) for their final PR review. `/linear:start` also uses the same procedure with `diffity --new main` when the user explicitly asks for a pre-commit review.
